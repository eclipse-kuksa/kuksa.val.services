...
DATABROKER_ADDRESS = os.environ.get("DATABROKER_ADDRESS",
                                    "127.0.0.1:55555")
...
KUKSA_QUERY = "SELECT {}, {}, {}".format(DP_BRAKE_POS,
                                         DP_ACCELR_POS,
                                         DP_STEER_ANGLE)

SIM_SPEED = os.environ.get("SIM_SPEED", 0.001)  # timeout between updates
...
async def main():
    ...
    while True:
        controls_update = asyncio.create_task(helper.subscribe_datapoints(
            KUKSA_QUERY, timeout=SIM_SPEED,
            sub_callback=set_model_param
        ))
        await controls_update
        car_sim.update_car()

        pub_speed = asyncio.create_task(
            helper.set_float_datapoint(DP_SPEED, car_sim.speed * 3.6) # 1 m/s = 3.6 km/h
        )   
