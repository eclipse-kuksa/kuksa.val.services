...
def main():
    ...
    def slideTemp(sender):
        ...
        # setting is performed here because the current temperature is set with this
        client.setTemp(value)

    with dpg.window(label="Demo Window"):
        dpg.add_slider_int(
            tag="TempSlider",
            label="current temperature in celsius",
            default_value=1,
            vertical=True,
            max_value=40,
            height=160,
        ...
        dpg.add_input_int(
            tag="TempRatio",
            label="Ratio to change temperature",
            default_value=30,
            min_clamped=True,
            min_value=1,
        )
