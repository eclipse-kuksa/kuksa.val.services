...
REPO_NAME="kuksa.val.services-carsim"
REPO_OWNER="mikehaller"
...
if [ "$DOCKER_ARCH" = "multiarch" ]; then
	...
	if [ $LOCAL -eq 1 ]; then
		...
		DOCKER_EXPORT="($DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/${REPO_OWNER}/${REPO_NAME}/$DOCKER_IMAGE:latest)"
	else
...
if [ $? -eq 0 ]; then
	echo "docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/${REPO_OWNER}/${REPO_NAME}/$DOCKER_IMAGE:latest"
	docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/${REPO_OWNER}/${REPO_NAME}/$DOCKER_IMAGE:latest
